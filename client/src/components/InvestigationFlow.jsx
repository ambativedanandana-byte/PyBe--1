...
import { getComicStory } from '../data/comicStories';
...
/* ─── Comic logic is now handled by external components/data ─── */
...
const ERR_BADGE = { 
  'Syntax Error': '#fee2e2,#fca5a5,#991b1b', 
  'Runtime Error': '#fff7ed,#fed7aa,#9a3412', 
  'Logical Error': '#eff6ff,#bfdbfe,#1e40af' 
};

// Standard fallback illustrations for stage 2 top row
const ILLUS_STAGE2 = {
...
export default function InvestigationFlow({ question, onAnswerSubmit, onNext }) {
  ...
  const story = getComicStory(question);
  ...
  useEffect(() => {
    setStage(1); 
    setS1ans([null,null,null]); 
    setS1done([false,false,false]);
    setChalIdx(0); 
    setChalDone([false,false,false]); 
    setAnalysisOn([false,false,false]);
    if (scenario) {
  ...
  useEffect(() => {
    if (stage === 3) {
      setConfetti(Array.from({length:36},(_,i) => ({ 
        ...
        color:['#fbbf24','#3b82f6','#10b981','#ec4899','#f97316'][i%5] 
      })));
    }
  }, [stage]);
  ...
  return (
    <div className="if2-wrap">
      ...
      {/* ══════════ STAGE 1: COMIC SCENARIO + 3 MCQS ══════════ */}
      {stage === 1 && (
        <div className="if2-s1">
          
          {/* Comic Book Strip Redesign for Stage 1 */}
          <ComicStrip story={story} question={question} />

          {/* LET'S THINK! Quiz Section */}
          <div className="cq-section">

            {/* Banner */}
            <div className="cq-banner">
              <span className="cq-banner-icon">💡</span>
              <span className="cq-banner-text">LET'S THINK!</span>
            </div>
            ...
            <div className="cq-grid">
              {step1.questions.map((qObj, qi) => {
                ...
                return (
                  <div key={qi} className={`cq-card ${done ? (ok ? 'cq-card-ok' : 'cq-card-no') : ''}`}>
                    {/* Question number badge */}
                    <div className="cq-qnum">{qi + 1}</div>
                    {/* Question text */}
                    <p className="cq-qtext">{qObj.q}</p>
                    {/* Options */}
                    <div className="cq-opts">
                      {qObj.options.map((opt, oi) => {
                        let cls = 'cq-opt';
                        if (done) {
                          if (oi === qObj.correct) cls += ' cq-opt-ok';
                          else if (oi === sel)       cls += ' cq-opt-no';
                        } else if (sel === oi) {
                          cls += ' cq-opt-sel';
                        }
                        return (
                          <button
                            key={oi}
                            className={cls}
                            onClick={() => pickS1(qi, oi)}
                            disabled={done}
                          >
                            <span className="cq-ltr">{String.fromCharCode(65 + oi)}</span>
                            <span className="cq-opt-txt">{opt}</span>
                          </button>
                        );
                      })}
                    </div>
                    {/* Inline feedback */}
                    {done && (
                      <div className={`cq-fb ${ok ? 'cq-fb-ok' : 'cq-fb-no'}`}>
                        {ok ? '✅ Correct!' : `❌ ${qObj.options[qObj.correct]}`}
                      </div>
                    )}
                    {/* Check button */}
                    {!done && (
            ...
            {/* Tip footer + Continue button row */}
            <div className="cq-footer">
              <div className="cq-tip">
                <span className="cq-tip-icon">💡</span>
                <span>Answer all 3 questions to continue to the next stage and find the bug in the code!</span>
              </div>
              <button
                className={`cq-continue-btn ${s1AllDone ? 'cq-continue-btn-on' : ''}`}
                onClick={() => s1AllDone && setStage(2)}
                disabled={!s1AllDone}
              >
        ...
        <div className="if2-s2">
          ...
          <div className="if2-chal-tabs">
            {step2.challenges.map((c, i) => (
              <button 
                key={i} 
                className={`if2-tab ${chalIdx === i ? 'if2-tab-on' : ''} ${chalDone[i] ? 'if2-tab-done' : ''}`} 
                onClick={() => setChalIdx(i)}
              >
            ...
            <div className="if2-mission-card">
              ...
              <div className="if2-mission-list">
                {['Find the bug in the code.', 'Fix the bug.', 'Run your code.', 'Pass all test cases.'].map((m, i) => (
                  <div key={i} className="if2-mission-item">
                    <CheckCircle2 size={14} color="#10b981"/> {m}
                  </div>
                ))}
              </div>
            </div>
            <div className="if2-illus-top">
              {ILLUS_STAGE2[scenario.illustration] || ILLUS_STAGE2.default}
            </div>
          </div>
          ...
          <div className={`if2-analysis-card ${curAnalysis ? 'if2-analysis-on' : ''}`}>
            <div className="if2-analysis-head">
              <DetectiveIcon/> Detective Analysis {!curAnalysis && <span className="if2-analysis-hint">(will appear after all tests pass)</span>}
            </div>
      ...
      {/* ══════════ STAGE 3: CASE CLOSED SUMMARY ══════════ */}
      {stage === 3 && (
        <div className="if2-s3">
          <div className="if2-confetti-wrap">
            {confetti.map(p => (
              <div key={p.id} className="if2-cp" style={{left: `${p.left}%`, animationDelay: `${p.delay}s`, background: p.color}}/>
            ))}
          </div>
          ...
          <div className="if2-celebrate">
            ...
            <p className="if2-cel-sub">
              You found and fixed all 3 bugs in the <b style={{color: '#fff', textTransform: 'capitalize'}}>{question.topic}</b> challenge.
            </p>
          </div>
          
          <div className="if2-sum-card">
            <div className="if2-sum-head"><Award size={17}/> What You Learned</div>
            {step3.learned.map((l, i) => (
              <div key={i} className="if2-learned">
                <CheckCircle2 size={14} color="#10b981"/> {l}
              </div>
            ))}
          </div>
          ...
          <div className="if2-tip-card">
            ...
            <div>
              <b className="if2-tip-title">Remember:</b>
              <p className="if2-tip-txt">{step3.tip}</p>
            </div>
          </div>
          
          <div className="if2-rewards">
            {[['✨', `+${question.level === 'Beginner' ? 10 : question.level === 'Explorer' ? 20 : 30} XP`, 'Earned'],
              ['🥇', 'Badge', 'Unlocked'],
              ['🔥', 'Streak', 'Continued'],
              ['📈', 'Progress', 'Updated']].map(([e, v, l], i) => (
              <div key={i} className="if2-rchip">
